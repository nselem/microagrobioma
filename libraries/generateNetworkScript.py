...
#table = load_table('/mnt/d/data/genomics/unam/agromicrobioma/pacific_ocean.biom')
    table = load_table(f"/mnt/d/data/genomics/unam/agromicrobioma/biom/maiz_nivel_{level}.biom")

#table = load_table('~/solena/biom/solena_maiz.biom')
    outName = f"maiz_nivel_{level}"
...
# https://biom-format.org/documentation/table_objects.html
    ...
    if loadNetworkFlag:
        network = statisticsFunctions.loadNetwork('solena_maiz_networkPval.csv')
    else:
...
finish = datetime.datetime.now()
statisticsFunctions.printNetwork(network,"solena_maiz_networkPermTest.csv")
...
finish = datetime.datetime.now()
statisticsFunctions.printNetwork(network,"solena_maiz_network_complete.csv")
